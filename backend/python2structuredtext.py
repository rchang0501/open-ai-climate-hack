...
def get_generated_response(response):
    generatedFile = "// Python Code generated from Structured Text Code: \n"
    while True:
...
def write_cpp_file(textResponse):
    fileName = PYTHON_FILE_TO_CONVERT.split(".")[0] + ".txt"
...
def iterate_for_compilable_solution(prompt, maxIterations):
    print('Codex is looking for a Stuctured Text solution ...')
